...
@app.route('/check_medication_notifications')
@login_required
def check_medication_notifications():
    """Manually trigger medication expiry check - admin only"""
    if current_user.role != 'admin':
        flash('Access denied')
        return redirect(url_for('home'))

    try:
        from medication_notifications import check_medication_expiry
        check_medication_expiry(mail, app.config)
        flash('Medication expiry check completed successfully.')

        audit_log = AuditLog(user_id=current_user.id, action="Ran medication expiry check")
        db.session.add(audit_log)
        db.session.commit()
    except Exception as e:
        flash(f'Error checking medication expiry: {str(e)}')
